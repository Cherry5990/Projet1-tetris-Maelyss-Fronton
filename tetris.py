...
START_PLABORD = int(MARGE[0]/2)-BORDURE_PLATEAU, MARGE[1]+BORDURE_PLATEAU # coordonnées auxquelles commence le plateau
...
PIECES = { # Dictionnaire stockant les représentations les 7 pièces possibles avec les rotations
	'O': [
		'0000\n0110\n0110\n0000',
	],
	'S': [
...
for name, rotations in PIECES.items():
	PIECES[name] = [[[int(i) for i in p] for p in r.splitlines()] for r in rotations] # Permet de transformer en int et séparer les chiffres dans les pièces
...
class Jeu:
	...
	def __init__(self):
		pygame.init() # lance tout les modules de pygame
...
#		print("Afficher Texte")
	...
	def _rendre(self): # met à jour l'affichage et avance l'horloge
		pygame.display.update() # peut servir à mettre à jour tout l'affichage ou une partie, sans argument met à jour tout l'affichage
		self.clock.tick()
...
#		print("Position testée valide: x=%s, y=%s" % (x, y))
	...
	def _poserPiece(self):
		...
		for i, line in enumerate(self.plateau[::-1]):
			for case in line:
				...
				print(self.plateau)
				print(">>> %s" % (DIM_PLATEAU[1]-1-i)) # numéro de la ligne qui a été complétées
	...
	def _first(self):
		self.plateau = [[0] * DIM_PLATEAU[0] for i in range(DIM_PLATEAU[1])] # plateau modélisé sous forme de liste de listes contenant les cases sous forme de numéro correspondant à une couleur
		self.score, self.pieces, self.lignes, self.tetris, self.niveau = 0, 0, 0, 0, 1
		self.current, self.next, self.perdu = None, self._getPiece(), False # pièce en cours, prochaine pièce

	def _next(self):
		print("Piece suivante")
		self.current, self.next = self.next, self._getPiece()
		self.pieces += 1
		self.position = [int(DIM_PLATEAU[0] / 2)-2, -4, 0] # position de la nouvelle pièce initialisée au milieu
	...
	def _dessinerPlateau(self):
		...
		for i, ligne in enumerate(self.plateau): # dessine le plateau et les pièces le contenant
